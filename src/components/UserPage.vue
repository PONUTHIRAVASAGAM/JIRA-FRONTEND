    ...
    <div class="component">
        <b-container>
            ...
            <div>
                ...
                <div>
                    <!--Update Card Modal Component -->
                    <b-modal id="updateCardModal" title="Upate Card" hide-header-close no-close-on-backdrop>
                        <div class="my-4">
                            <b-form-group label="Card Name:" label-for="input-text">
                                <b-form-input id="input-text" v-model="card.card_name" placeholder="Card Name" required></b-form-input>
                            </b-form-group>
                            <br>
                            <b-form-group label="Description" label-for="description-textarea">
                                <b-form-textarea id="description-textarea" v-model="card.description" placeholder="Enter a description" rows="3" max-rows="6"></b-form-textarea>
                            </b-form-group>
                        </div>
                        <template #modal-footer>
                            <b-button variant="secondary" @click="cancelUpdateCard()">Cancel</b-button>
                            <b-button variant="primary" @click="updateCard(card.id,card.card_name,card.description)">Update</b-button>
                        </template>
                    </b-modal>
                </div>
                ...
                <div class="columns-container">
                    ...
                    <div id="list" v-for="(column, colIndex) in columns" :key="column.listId" class="column" :data-column-id="column.listId">
                        <!-- {{ column }} -->
                        ...
                        <draggable :list="column.cards" group="cards" @end="onCardDrop($event)" class="drag-area">
                            ...
                            <b-card id="card" v-for="(card, cardIndex) in column.cards" :key="cardIndex" footer-tag="footer" :data-card-id="card.id">
                                <template v-slot:header>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <h6 class="mb-0"><b>{{ card.card_name }}</b></h6>
                                        <div class="d-flex align-items-center">
                                            <b-icon id="pencil" icon="pencil" scale="1" class="text-primary update-icon me-2" v-b-modal.updateCardModal @click="getCard(card)"></b-icon>
                                            <b-icon icon="x" scale="1.5" class="text-danger close-icon" @click="confirmDeleteCard(cardIndex, card.id)"></b-icon>
                                        </div>
                                    </div>
                                </template>
                            ...
                            <!-- <b-card id="card" v-for="(card, cardIndex) in column.cards" :key="cardIndex" :header="card.card_name" header-tag="header" footer-tag="footer" :data-card-id="card.id"> -->
                            <!-- <b-icon icon="pencil" scale="1" class="text-primary update-icon" @click="updateCard(cardIndex, card.id)"> -->
                            <!-- </b-icon> -->
        ...
        methods: {
            ...
            updateCard(cardId, cardName, cardDescription) {
                ...
                if (cardName == null || cardName == "") {
                    ...
                    console.log("=====Else Calling======")
                    this.$bvModal.hide('updateCardModal');
                    const newColumn = {
                        card_name: cardName,
                        description: cardDescription,
                        card_id: cardId,
                    ...
                    ax
                        .put("api/cards/updateCardData", newColumn)
                        .then((response) => {
                            Swal.fire({
                                title: 'Success!',
